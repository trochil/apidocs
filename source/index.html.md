...
蜂鸟数据是轻量级金融终端，提供主流金融市场的实时报价和历史数据。

蜂鸟数据已经开启链上预言机开发，蜂鸟数据希望成为加密世界与现实世界的连接器，通过为加密世界提供合约可以直接调用的数据，令加密世界服务于每一个普通人。

我们提供高度统一和强大的REST API和Websocket API，让您轻松获取金融数据。
